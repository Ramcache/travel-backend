...
const hotelFields = `
	id, name, city, stars, distance, distance_text, meals, guests, photo_url, transfer, created_at, updated_at
`
...
func (r *HotelRepository) Update(ctx context.Context, hotel *models.Hotel) error {
	query := `UPDATE hotels 
              SET name=$1, city=$2, stars=$3, distance=$4, distance_text=$5,
                  meals=$6, guests=$7, photo_url=$8, transfer=$9, updated_at=now() 
              WHERE id=$10
...
func (r *HotelRepository) ListByTrip(ctx context.Context, tripID int) ([]models.Hotel, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&h.ID, &h.Name, &h.City, &h.Stars, &h.Distance, &h.DistanceText,
			&h.Meals, &h.Guests, &h.PhotoURL, &h.Transfer, &h.CreatedAt, &h.UpdatedAt, &h.Nights,
		); err != nil {
			return nil, err
		}
		hotels = append(hotels, h)
